...
const AIChatAssistant = ({ isOpen, onClose, onSubmit }) => {
  const [messages, setMessages] = useState([
    { text: "Hello! I'm BookVerse AI. How can I help you find your next great read?", sender: 'ai',loading :false }
  ]);
  ...
  const handleSubmit = async (e) => {
    ...
    setInputValue('');
    setIsTyping(true);
//add temporaray loading message 
setMessages(prev=>[...prev,{
  ...
  loading:true
    ...
    try {
      ...
      setMessages(prev => [...prev.slice(0.-1), { text: aiResponse, sender: 'Ai',loading:false }]);
    } catch (error) {
      console.error("Chat error:",error);
      setMessages(prev => [...prev.slice(0,-1), { text: "Sorry, I encountered an error. Please try again.", sender: 'Ai',loading :false }]);
    } finally {
      setIsTyping(false);
    }
  };
...
useEffect(()=>{
    if(isTyping){
        let i=0;
        const responseText="I'm thinking....";// replace with actual response
        ...
        >
          <div className="bg-indigo-600 text-white p-4 flex justify-between items-center">
            <div className="flex items-center">
              ...
              <h3 className="font-bold">BookVerse AI Assistant</h3>
            </div>
          ...
          <form onSubmit={handleSubmit} className="border-t border-gray-200 p-4">
            ...
            <p className="text-xs text-gray-500 mt-2">
              Try: "Recommend me a fantasy book" or "What's popular in science fiction?"
            </p>
          </form>
        </motion.div>
      )}
